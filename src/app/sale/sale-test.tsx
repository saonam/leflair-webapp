...
import { ProductCardTest } from '../product';
...
export const saleAction = async (params?: any, cookie?: any, store?: any) => {
  ...
  const { sort, ...filter }: any = typeof window === 'undefined' ? {} : qsToJson(location.search);
  try {
    const sale: SaleProps = await getSale(saleId, filter, sort, store);
...
const Product = ({ data }: any) => (
  <ProductCardTest product={data} isSaleProduct={true}/>
);
...
class SaleComponent extends Component<any, any> {
  ...
  constructor(props: { sale: SaleProps, isSignedIn: boolean, isSubscribed: boolean, store: any }) {
    ...
    this.state = {
      ...
      isFiltersCollapsed: filter ? !(Object.keys(filter).length && typeof window !== 'undefined' && window.innerWidth >= BREAKPOINT_MAX['sm']) : true,
  ...
  renderProductCard(product: any, index: number) {
    return (
      <div key={index} className={`${styles.col6} ${this.state.isFiltersCollapsed ? `${styles.colMd4} ${styles.colLg3}` : `${styles.colMd5} ${styles.colLg4}`} ${styles.variant}`}>
  ...
  componentDidMount() {
    ...
    if (!this.state.isFiltersCollapsed) {
